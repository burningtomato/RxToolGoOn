...
import com.bt.dialog.util.RxTimeTool.getDaysByYearMonth
import com.bt.dialog.wheel.NumericWheelAdapter
import com.bt.dialog.wheel.OnWheelChangedListener
import com.bt.dialog.wheel.WheelView
import com.xlkj.mylibrary.R
import java.util.*
...
class RxDialogDate : RxDialog {
    ...
    private fun build() {
        ...
        val dialogView1 =
            LayoutInflater.from(mContext).inflate(R.layout.dialog_year_month_day, null)
        ...
        yearView.setViewAdapter0(DateNumericAdapter(mContext, beginYear, endYear, curYear))
        yearView.currentItem = (endYear-beginYear)/2
        ...
        monthView.setViewAdapter0(
            DateNumericAdapter(
                mContext,
                months[0].toInt(),
                months[months.size - 1].toInt(),
                months[curMonth].toInt()
            )
        )
...
//        val maxDay = mCalendar.getActualMaximum(Calendar.DAY_OF_MONTH)
     ...
     */
    private inner class DateNumericAdapter(
        mContext: Context?, minValue: Int, maxValue: Int, // Index of item to be highlighted
        var currentValue: Int
    ) : NumericWheelAdapter(mContext!!, minValue, maxValue) {
