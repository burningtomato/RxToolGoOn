...
public class TCrashTool {
    ...
    private static final String EXTRA_ACTIVITY_TLog = "com.tamsiree.rxkit.crash.tcrashtool.EXTRA_ACTIVITY_TLog";
    ...
    private static final int MAX_ACTIVITIES_IN_TLog = 50;

    //Shared preferences
    private static final String SHARED_PREFERENCES_FILE = "TCrashTool";
    private static final String SHARED_PREFERENCES_FIELD_TIMESTAMP = "last_crash_timestamp";
    private static final Deque<String> activityTLog = new ArrayDeque<>(MAX_ACTIVITIES_IN_TLog);
    //Internal variables
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(TAM_HANDLER_PACKAGE_NAME)) {
                            ...
                            if (hasCrashedInTheLastSeconds(application)) {
                                ...
                                if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                                    ...
                                    if (config.isTrackActivities()) {
                                        StringBuilder activityTLogStringBuilder = new StringBuilder();
                                        while (!activityTLog.isEmpty()) {
                                            activityTLogStringBuilder.append(activityTLog.poll());
                                        }
                                        intent.putExtra(EXTRA_ACTIVITY_TLog, activityTLogStringBuilder.toString());
                                    }
                    ...
                    application.registerActivityLifecycleCallbacks(new Application.ActivityLifecycleCallbacks() {
                        ...
                        public void onActivityCreated(@NonNull Activity activity, Bundle savedInstanceState) {
                            ...
                            if (config.isTrackActivities()) {
                                activityTLog.add(dateFormat.format(new Date()) + ": " + activity.getClass().getSimpleName() + " created\n");
                            }
                        }
                        ...
                        public void onActivityResumed(@NonNull Activity activity) {
                            if (config.isTrackActivities()) {
                                activityTLog.add(dateFormat.format(new Date()) + ": " + activity.getClass().getSimpleName() + " resumed\n");
                            }
                        }
                        ...
                        public void onActivityPaused(@NonNull Activity activity) {
                            if (config.isTrackActivities()) {
                                activityTLog.add(dateFormat.format(new Date()) + ": " + activity.getClass().getSimpleName() + " paused\n");
                            }
                        }
                        ...
                        public void onActivityDestroyed(@NonNull Activity activity) {
                            if (config.isTrackActivities()) {
                                activityTLog.add(dateFormat.format(new Date()) + ": " + activity.getClass().getSimpleName() + " destroyed\n");
                            }
                        }
                    });
                }
    ...
    /**
     * Given an Intent, returns the activity TLog extra from it.
     *
     * @param intent The Intent. Must not be null.
     * @return The activity TLog, or null if not provided.
     */
    @Nullable
    public static String getActivityTLogFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(TCrashTool.EXTRA_ACTIVITY_TLog);
    }
    ...
    public static String getAllErrorDetailsFromIntent(@NonNull Context context, @NonNull Intent intent) {
        ...
        String activityTLog = getActivityTLogFromIntent(intent);

        if (activityTLog != null) {
            errorDetails += "\nUser actions : \n";
            errorDetails += activityTLog;
        }
        return errorDetails;
    }
...
////            String s = packageInfo.applicationInfo.toString();
////            System.out.println("packageInfo.applicationInfo = " + packageInfo.applicationInfo);
////            return s;
    ...
    public static String getAppName(Context context) {
        ...
        if (isNullString(packageName)) {
            ...
            if (pi.applicationInfo != null) {
                CharSequence charSequence = pi.applicationInfo.loadLabel(pm);
    ...
    private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(@NonNull Context context) {
        ...
        if (resolveInfos.size() > 0) {
            ...
            try {
                ...
                //Should not happen, print it to the TLog!
